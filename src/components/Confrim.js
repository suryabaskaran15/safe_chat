...
import { deleteField, doc, getDoc, query, updateDoc, where } from "firebase/firestore";
...
const Confrim = (props) => {
  ...
  const getMsgFromDb = async()=>{
    const msg = await (await getDoc(doc(db,'user',auth.currentUser.uid,'messages',details.reciverUid))).data()
  ...
  const editMsg = async()=>{
    await updateDoc(doc(db,'user',auth.currentUser.uid,'messages',details.reciverUid),{
      [details.msgId]: {
    ...
    await updateDoc(doc(db,'user',details.reciverUid,'messages',auth.currentUser.uid),{
      [details.msgId]: {
      date : msgDetails.date,
      from : msgDetails.from,
      msg: alteredMsg,
    ...
    <Modal show={details.show}>
      ...
      <ModalBody>
        ...
        <Input 
            ...
            id='editedMsg' 
            value={alteredMsg} 
            onChange={(e)=>{setAlteredMsg(e.target.value)}}
            />
        </form>
        :"Delete"}
      </ModalBody>
      <ModalFooter>
        ...
        <Button onClick={details.option === "edit" ? ()=>{
          editMsg();
          setDetails({ show: false });
          props.getShow({show:false});
          } : ()=>{
